from __future__ import annotations

import re
from dataclasses import dataclass
from types import MappingProxyType
from typing import Iterable, Mapping, Sequence


class SecretExposureError(ValueError):
    """Raised when secret exposure validation fails."""


_REDACTED = "[REDACTED]"

_SECRET_NAME_MARKERS = (
    "PASSWORD",
    "PASSWD",
    "SECRET",
    "TOKEN",
    "API_KEY",
    "APIKEY",
    "PRIVATE_KEY",
    "PRIVATEKEY",
    "CREDENTIAL",
    "CREDENTIALS",
    "AUTH",
    "ACCESS_KEY",
    "ACCESSKEY",
)

_SECRET_ARGUMENT_NAMES = {
    "--password",
    "--passwd",
    "--secret",
    "--token",
    "--api-key",
    "--apikey",
    "--private-key",
    "--credential",
    "--credentials",
    "--auth",
    "--access-key",
}

_CONTROL_RE = re.compile(r"[\x00-\x1f\x7f]")


@dataclass(frozen=True)
class SecretExposurePolicy:
    reject_secret_named_environment: bool = True
    redact_secret_named_environment: bool = True
    reject_control_characters: bool = True
    max_command_argument_length: int = 4096
    max_environment_name_length: int = 256
    max_environment_value_length: int = 8192
    redaction_marker: str = _REDACTED

    def __post_init__(self) -> None:
        if not isinstance(self.reject_secret_named_environment, bool):
            raise TypeError("reject_secret_named_environment must be bool")

        if not isinstance(self.redact_secret_named_environment, bool):
            raise TypeError("redact_secret_named_environment must be bool")

        if not isinstance(self.reject_control_characters, bool):
            raise TypeError("reject_control_characters must be bool")

        if (
            isinstance(self.max_command_argument_length, bool)
            or not isinstance(self.max_command_argument_length, int)
        ):
            raise TypeError("max_command_argument_length must be int")

        if (
            isinstance(self.max_environment_name_length, bool)
            or not isinstance(self.max_environment_name_length, int)
        ):
            raise TypeError("max_environment_name_length must be int")

        if (
            isinstance(self.max_environment_value_length, bool)
            or not isinstance(self.max_environment_value_length, int)
        ):
            raise TypeError("max_environment_value_length must be int")

        if self.max_command_argument_length <= 0:
            raise ValueError(
                "max_command_argument_length must be positive"
            )

        if self.max_environment_name_length <= 0:
            raise ValueError(
                "max_environment_name_length must be positive"
            )

        if self.max_environment_value_length <= 0:
            raise ValueError(
                "max_environment_value_length must be positive"
            )

        if not isinstance(self.redaction_marker, str):
            raise TypeError("redaction_marker must be str")

        if not self.redaction_marker:
            raise ValueError("redaction_marker must not be empty")

        if _CONTROL_RE.search(self.redaction_marker):
            raise ValueError(
                "redaction_marker contains control characters"
            )


@dataclass(frozen=True)
class SecretExposureResult:
    safe: bool
    failures: tuple[str, ...]
    redacted_command: tuple[str, ...]
    redacted_environment: Mapping[str, str]

    def __post_init__(self) -> None:
        if not isinstance(self.safe, bool):
            raise TypeError("safe must be bool")

        object.__setattr__(
            self,
            "failures",
            tuple(self.failures),
        )

        object.__setattr__(
            self,
            "redacted_command",
            tuple(self.redacted_command),
        )

        object.__setattr__(
            self,
            "redacted_environment",
            MappingProxyType(dict(self.redacted_environment)),
        )

    @property
    def exposed(self) -> bool:
        return not self.safe

    def to_dict(self) -> dict[str, object]:
        return {
            "safe": self.safe,
            "failures": list(self.failures),
            "redacted_command": list(self.redacted_command),
            "redacted_environment": dict(
                self.redacted_environment
            ),
        }


def _is_secret_name(name: str) -> bool:
    normalized = name.upper()

    return any(
        marker in normalized
        for marker in _SECRET_NAME_MARKERS
    )


def _is_secret_argument(argument: str) -> bool:
    normalized = argument.strip().lower()

    if normalized in _SECRET_ARGUMENT_NAMES:
        return True

    for name in _SECRET_ARGUMENT_NAMES:
        prefix = name + "="

        if normalized.startswith(prefix):
            return True

    return False


def _validate_text(
    value: str,
    *,
    field: str,
    reject_control_characters: bool,
) -> list[str]:
    if not reject_control_characters:
        return []

    if "\x00" in value:
        return [f"{field}:NULL_CHARACTER"]

    if _CONTROL_RE.search(value):
        return [f"{field}:CONTROL_CHARACTER"]

    return []


def _normalize_secret_values(
    secret_values: Iterable[str] | None,
) -> tuple[str, ...]:
    if secret_values is None:
        return ()

    if isinstance(secret_values, (str, bytes)):
        raise TypeError(
            "secret_values must be an iterable of strings"
        )

    values: list[str] = []

    for value in secret_values:
        if not isinstance(value, str):
            raise TypeError(
                "every secret value must be a string"
            )

        if value:
            values.append(value)

    return tuple(
        dict.fromkeys(values)
    )


def _redact_known_secrets(
    value: str,
    secret_values: tuple[str, ...],
    marker: str,
) -> str:
    result = value

    for secret in sorted(
        secret_values,
        key=len,
        reverse=True,
    ):
        result = result.replace(
            secret,
            marker,
        )

    return result


def _redact_argument(
    argument: str,
    *,
    secret_values: tuple[str, ...],
    marker: str,
) -> str:
    result = _redact_known_secrets(
        argument,
        secret_values,
        marker,
    )

    if "=" in result:
        name, value = result.split("=", 1)

        if _is_secret_argument(name):
            return f"{name}={marker}"

    return result


def validate_secret_exposure(
    command: Sequence[str],
    environment: Mapping[str, str],
    *,
    secret_values: Iterable[str] | None = None,
    policy: SecretExposurePolicy | None = None,
) -> SecretExposureResult:
    """
    Validate command/environment for secret exposure.

    This function does not execute commands and does not modify the
    supplied command or environment.
    """

    if policy is None:
        policy = SecretExposurePolicy()

    if isinstance(command, (str, bytes)):
        raise TypeError(
            "command must be a sequence of string arguments"
        )

    if not isinstance(environment, Mapping):
        raise TypeError(
            "environment must be a mapping"
        )

    secrets = _normalize_secret_values(
        secret_values
    )

    failures: list[str] = []
    redacted_command: list[str] = []

    command_items = list(command)

    for index, argument in enumerate(command_items):
        if not isinstance(argument, str):
            raise TypeError(
                "every command argument must be a string"
            )

        if len(argument) > policy.max_command_argument_length:
            failures.append(
                f"COMMAND_ARGUMENT_TOO_LONG:{index}"
            )

        failures.extend(
            _validate_text(
                argument,
                field=f"COMMAND_ARGUMENT:{index}",
                reject_control_characters=(
                    policy.reject_control_characters
                ),
            )
        )

        if any(
            secret in argument
            for secret in secrets
            if secret
        ):
            failures.append(
                f"SECRET_EXPOSED_IN_COMMAND:{index}"
            )

        if _is_secret_argument(argument):
            failures.append(
                f"SECRET_ARGUMENT_NAME:{index}"
            )

        if "=" in argument:
            argument_name = argument.split("=", 1)[0]

            if _is_secret_argument(argument_name):
                failures.append(
                    f"SECRET_ARGUMENT_NAME:{index}"
                )

        redacted_command.append(
            _redact_argument(
                argument,
                secret_values=secrets,
                marker=policy.redaction_marker,
            )
        )

    redacted_environment: dict[str, str] = {}

    for name, value in environment.items():
        if not isinstance(name, str):
            raise TypeError(
                "environment variable names must be strings"
            )

        if not isinstance(value, str):
            raise TypeError(
                "environment variable values must be strings"
            )

        if len(name) > policy.max_environment_name_length:
            failures.append(
                f"ENVIRONMENT_NAME_TOO_LONG:{name}"
            )

        if len(value) > policy.max_environment_value_length:
            failures.append(
                f"ENVIRONMENT_VALUE_TOO_LONG:{name}"
            )

        failures.extend(
            _validate_text(
                name,
                field=f"ENVIRONMENT_NAME:{name}",
                reject_control_characters=(
                    policy.reject_control_characters
                ),
            )
        )

        failures.extend(
            _validate_text(
                value,
                field=f"ENVIRONMENT_VALUE:{name}",
                reject_control_characters=(
                    policy.reject_control_characters
                ),
            )
        )

        secret_named = _is_secret_name(name)

        if (
            secret_named
            and policy.reject_secret_named_environment
            and value
        ):
            failures.append(
                f"SECRET_NAMED_ENVIRONMENT:{name}"
            )

        if any(
            secret in value
            for secret in secrets
            if secret
        ):
            failures.append(
                f"SECRET_EXPOSED_IN_ENVIRONMENT:{name}"
            )

        if (
            secret_named
            and policy.redact_secret_named_environment
        ):
            redacted_value = policy.redaction_marker
        else:
            redacted_value = _redact_known_secrets(
                value,
                secrets,
                policy.redaction_marker,
            )

        redacted_environment[name] = redacted_value

    unique_failures = tuple(
        dict.fromkeys(failures)
    )

    return SecretExposureResult(
        safe=not unique_failures,
        failures=unique_failures,
        redacted_command=tuple(redacted_command),
        redacted_environment=redacted_environment,
    )


def validate_secret_exposure_vector(
    command: Sequence[str],
    environment: Mapping[str, str],
    *,
    secret_values: Iterable[str] | None = None,
    policy: SecretExposurePolicy | None = None,
) -> SecretExposureResult:
    return validate_secret_exposure(
        command,
        environment,
        secret_values=secret_values,
        policy=policy,
    )


def require_no_secret_exposure(
    command: Sequence[str],
    environment: Mapping[str, str],
    *,
    secret_values: Iterable[str] | None = None,
    policy: SecretExposurePolicy | None = None,
) -> SecretExposureResult:
    result = validate_secret_exposure(
        command,
        environment,
        secret_values=secret_values,
        policy=policy,
    )

    if not result.safe:
        raise SecretExposureError(
            "secret exposure validation failed: "
            + ", ".join(result.failures)
        )

    return result
